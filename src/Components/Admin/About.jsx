...
const About = ({ placeholder }) => {
    ...
    const { register, handleSubmit, formState: { errors } } = useForm({
        defaultValues: async () => {
            try {
                const res = await axios.get(`${apiUrl}/about-us/${params.id}`, {
                    headers: {
    ...
    const onSubmit = async (data) => {
        ...
        setLoader(true)
        try {
            const res = await axios.put(`${apiUrl}/about-us/${params.id}`, newData, {
                headers: {
                    Authorization: `Bearer ${token()}`
                }
            })
                    ...
                    <form className="md:grid md:grid-cols-2 gap-8" onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="col-span-2 flex flex-col gap-3">
                            ...
                            <div className="w-[100px]">
                                {
                                    aboutUs.image && <img src={`${fileUrl}/uploads/abouts/small/${aboutUs.image}`} />
                                }
                            </div>
