...
const {register,handleSubmit,formState: {errors}} = useForm({
    defaultValues: async() => {
     try{
        const res = await axios.get(`${apiUrl}/members/${params.id}`,{
            headers : {
...
const onSubmit = async(data) => {
   ...
   setLoader(true)
   try{
       const res = await axios.put(`${apiUrl}/members/${params.id}`,newData,{
          headers : {
            Authorization: `Bearer ${token()}`
          }
       })
...
<div className="mt-8 bg-white p-8 rounded-lg">
    <form className="md:grid md:grid-cols-2 gap-8" onSubmit={handleSubmit(onSubmit)}>
       <div>
            ...
            <input {...register('name',{required:"The Name field is required"})} type="text" name="name" className={`w-full p-2.5 mt-3 focus:outline-none border border-gray-300 rounded ${errors.name && 'invalid:border border-pink-500'}`} placeholder="Enter Name"/>
           ...
           <div>
            ...
            <input {...register('job_title',{required:"The Job Title field is required"})} type="text" name="job_title" className={`w-full p-2.5 mt-3 focus:outline-none border border-gray-300 rounded ${errors.job_title && 'invalid:border border-pink-500'}`} placeholder="Enter Job Title"/>
