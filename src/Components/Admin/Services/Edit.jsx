...
const Edit = ({ placeholder }) => {
   ...
   const { register, handleSubmit, formState: { errors } } = useForm({
      defaultValues: async () => {
         try {
            const res = await axios.get(`${apiUrl}/services/${params.id}`, {
               headers: {
   ...
   const onSubmit = async (data) => {
      ...
      setLoader(true)
      try {
         const res = await axios.put(`${apiUrl}/services/${params.id}`, newData, {
            headers: {
               Authorization: `Bearer ${token()}`
            }
         })
            ...
            <div className="mt-8 bg-white p-8 rounded-lg">
               <form className="md:grid md:grid-cols-2 gap-8" onSubmit={handleSubmit(onSubmit)}>
                  ...
                  <div className="col-span-2 flex flex-col gap-3">
                     ...
                     <div className='w-[200px] object-cover cursor-pointer'>
                        {
